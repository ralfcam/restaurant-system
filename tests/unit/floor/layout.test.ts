...
function readAutoAssignSource() {
  return readFileSync(
    path.join(repoRoot, "lib/reservations/auto-assign.ts"),
    "utf8",
  )
}
...
describe("floor layout grid", () => {
  it("snaps pointer coordinates to a cell and finds the next free slot", () => {
    ...
    expect(floorCanvasCells([{ x: 3, y: 2 }])).toMatchObject({
      cols: 5,
      rows: 4,
    })
    expect(
  ...
  it("spreads stacked default cells onto the seed dining-room grid", () => {
    ...
    expect(spread.find((table) => table.label === "1")).toMatchObject(
      SEED_FLOOR_LAYOUT["1"],
    )
    expect(spread.find((table) => table.label === "2")).toMatchObject(
      SEED_FLOOR_LAYOUT["2"],
    )
    expect(spread.find((table) => table.label === "5")).toMatchObject(
      SEED_FLOOR_LAYOUT["5"],
    )
    expect(spread.find((table) => table.label === "11")).toMatchObject({
      x: 2,
      y: 0,
    })
  ...
  it("floor table view type includes id x y for spreadOverlappingTables", () => {
    const source = readAutoAssignSource()
    const assignableTable =
      source.match(/export type AssignableTable\s*=\s*\{[\s\S]*?\n\}/)?.[0] ??
      ""
    const floorTableView =
      source.match(
        /export type FloorTableView(?:<[^>]*>)?\s*=\s*[\s\S]*?\n\}/,
      )?.[0] ?? ""
    const liveFloorTableTypes = `${assignableTable}\n${floorTableView}`
