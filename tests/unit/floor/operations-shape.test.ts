...
describe("table create and seat updates persist parity shape", () => {
  beforeEach(() => {
    ...
    mocks.selectCurrent.mockResolvedValue({
      data: { status: "available", x: 0, y: 0 },
      error: null,
    })
  ...
  it("writes round when capacity becomes odd and square when even", async () => {
    ...
    expect(mocks.update).toHaveBeenCalledWith(
      expect.objectContaining({ seats: 3, shape: "round" }),
    )
    await updateTableState({ id: "t1", seats: 4 })
    expect(mocks.update).toHaveBeenCalledWith(
      expect.objectContaining({ seats: 4, shape: "square" }),
    )
  })
  ...
  it("persists a spread when stacked tables share the origin cell", async () => {
    ...
    expect(
      tables.map((table) => ({ label: table.label, x: table.x, y: table.y })),
    ).toEqual([
      { label: "1", x: 0, y: 0 },
      { label: "2", x: 1, y: 0 },
    ])
  ...
  it("persists clamped floor coordinates", async () => {
    ...
    expect(mocks.update).toHaveBeenCalledWith(
      expect.objectContaining({ x: 11, y: 0 }),
    )
  })
})
