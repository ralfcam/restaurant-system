...
service-availability overview (WA-1–WA-7), the `/pos` table
...
2. **Blocked dates (staff)** — Staff blocked-date management respects service-role
   ...
   (§17), not only that RLS policy. Authenticated write policy and guest table
   privileges for `blocked_dates` are specified by §19 SIB-PRIV.
...
4. **FP-1 — Persisted floor tables** — Schema defines `tables` (`label`, `seats`,
   `status`, `x`, `y`, `shape`) with staff/service-role access (§19 SIB-PRIV:
   no `PUBLIC`/`anon`/`authenticated` table privilege or policy; staff reads
   and writes are `requireStaffUser` plus `createServiceClient`). Seed loads the
...
7. **FP-4 — Live Floor Plan via hooks** — `/admin/floor` is a live view. The
   ...
   displays as reserved; `seated` displays as seated. The dining-room chip MUST
   render the occupying reservation’s guest name, party size, and reservation
   `time` (the same `time` field the inspector shows). Those three fields MUST
   come from the reservation assigned to that table (or the merge primary per
   FP-8). A table with no occupying overlay MUST NOT show guest name, party
   size, or reservation time. **FP-4-PARTY** — The party-size figure MUST be
   the occupying reservation’s `party_size` and MUST NOT fall back to
   `tables.seats`. Unassigned chips MAY keep a non-party capacity cue (chip
   size class) but MUST NOT render `seats` in the party-size slot. The hook
   runs auto-assign on each refresh so a table appears at the proper time
   without a manual dropdown. Tonight’s book helper copy describes auto-assign
   as booked time minus expected turn (default 90), not a 15-minute lead.
...
8. **FP-5 — Status stays in sync** — Seating a reservation sets its assigned
   ...
   whose BW-9 occupying window overlaps the candidate — computed from the restaurant-wide `restaurant_settings.occupancy_duration_minutes` + `safety_buffer_minutes` (the same clamped values `assignReservationTable` uses), not the 90 + 15 defaults unless the settings row is absent. Floor `tables.status`
   ...
   `cancelled`, `no_show`) stay unassignable. **FP-5-SETTINGS-READ-FAIL** — A
   `restaurant_settings` SELECT error is not an absent row. On such an error,
   `assignReservationTable` MUST log server-side and refuse a new non-null
   label with `errors.reservation.assignFailed`, writing nothing to
   `reservations` or `tables`. Clearing `table_label` (or re-selecting the
   current label) stays allowed. The dropdown MAY keep its 90 + 15 display
   fallback in that case, because the write path is authoritative.
...
11. **FP-8 — Temporary table merges** — Admin can merge two or more
    ...
    **Available** and **Out of service** dissolve the merge; **Reserved**
    ...
    `table_merges`, `table_merge_members`, and `status_events` follow
    §19 SIB-PRIV (service-role-only table privilege and policy).
...
12. **FP-9 — Room layout and move-lock** — `/admin/floor` places each table
    ...
    - **FP-9-FOLLOW — Continuous drag.** Once an unlocked table passes the
      8px drag threshold, its rendered position tracks the pointer delta in
      CSS pixels, as a translate from its origin cell. It MUST NOT be rounded
      to grid cells before release, and per-move handling MUST NOT write draft
      cell coordinates.
    - **FP-9-DROP — Grab-offset-preserving snap.** The drop cell is the
      nearest grid cell to the chip's dragged position:
      `origin + round(delta / FLOOR_CELL_PX)` per axis, clamped to the grid.
      The grab offset is therefore preserved. A drag shorter than half a cell
      on both axes resolves to the origin: no persist, no split. This same
      cell drives the merge-target highlight during the drag and the drop
      outcome: merge onto an occupant (FP-8), split a merged member dropped
      on an empty cell, otherwise persist via `updateTableState`. The
      persisted position survives a reload.
    - **FP-9-TOUCH — Touch drags are not taken over by scrolling.** A movable
      chip (edit mode + unlocked + not merging) sets `touch-action: none`.
      Locked chips and service-mode chips keep the default `touch-action`, so
      the floor still pans on mobile. Locked tables stay non-draggable.
...
13. **Guest note on opening-hour segments** — Staff can set an optional
    **guest note** per opening-hour segment on `/admin/scheduling`. It is stored
    ...
    - **OH-NOTE-SAVE — Bounded save:** after existing whitespace trimming, a
      non-empty `operating_windows.guest_note` is at most 240 JavaScript string
      characters. `validateOperatingDays` and therefore
      `upsertOperatingWindows` reject a longer note before
      `replace_operating_windows`; exactly 240 is accepted. Rejection is
      explicit, never truncation. Blank/whitespace omission remains unchanged.
    - **OH-NOTE-INPUT — Matching staff control:** each opening-segment
      guest-note input uses the exported `MAX_GUEST_NOTE_LENGTH` as
      `maxLength`, with that constant equal to 240. This is a staff
      affordance; OH-NOTE-SAVE remains the authoritative trust boundary. The
...
27. **CL-1 — Bookable slots per service** — Staff can define an ordered list
    ...
    **non-empty** list is the exclusive offer set for that segment:
    ...
    times in one segment are rejected. **CL-1-DUP** — Duplicates are compared
    after `HH:MM` normalization (`19:00` and `19:00:00` are the same time).
    `validateOperatingDays` returns `errors.scheduling.duplicateSlotTime` with
    `{ day }` before `replace_operating_windows`. **CL-1-INTERVAL** — When any
    open segment in the payload has a non-empty `bookable_slots` list,
    `upsertOperatingWindows` MUST read `restaurant_settings.slot_interval_minutes`
    (id 1) and validate against its clamped value. An absent row or a null
    column uses the FP-10 default of 30. On a SELECT error, it MUST log
    server-side, MUST NOT call `replace_operating_windows`, and returns
    `errors.availability.settingsLoadFailed`. The `/admin/scheduling` Save
    preview in `SchedulingManager` MUST validate against the same configured
    interval, passed in from the page, so a time on that grid does not disable
    Save.
...
34. **FP-13 — POS table picker is live floor inventory** — `/pos`'s Table
    ...
    page load. When there are no live tables (`tables.length` is 0), the
...
35. **FP-14 — POS server picker is live server inventory** — A persisted
    ...
    grant; mirroring `tables`), tagged `-- REAZED-329`. `supabase/seed.sql` MUST seed the four names Maya, Jon,
    Priya, and Dev. `getServers()` in `app/actions/operations.ts` MUST
    mirror `getTables()`'s staff gate (`requireStaffUser()` → `[]` for a
    non-staff session; mapped `{id, name}[]` ordered by name for staff).
    `PosTerminal`'s Server `Select` MUST source from a `servers` prop, not
    the static `SERVERS` array in `lib/data.ts`. `app/pos/page.tsx` fetches
    `getServers()` and passes `<PosTerminal … servers={servers} />`. When
    `servers.length` is 0, the Server `Select` MUST disable and show a
...
36. **FP-15 — Seated bill total on the chip** — When the overlay status is
    ...
    plus two decimal places). A seated table with no such orders shows
    `CHF 0.00`. Confirmed (not seated) overlays MUST NOT show a bill.
    Unassigned tables MUST NOT show a bill. Totals ride on the same
    `getFloorSnapshot` payload and `useFloorPlan` 5s SWR refresh as tables
    and reservations — staff MUST NOT need a manual reload to see a new POS
    send. Added overlay lines MUST remain readable and MUST NOT replace or
    hide `tables.label` or the visual status (status color, seated ping).
    Walk-in seating without a reservation overlay is out of scope (RES-83).
    **FP-15-COMPLETE** — The snapshot MUST include every matching order, not
    only the first PostgREST page (`max_rows` = 1000 in
    `supabase/config.toml`). `getFloorSnapshot` MUST keep requesting the next
    page until a page has fewer than 1000 rows (or use one server-side
    per-`table_label` sum that cannot truncate line items). Each paged
    `orders` read MUST apply a unique immutable order on `orders.id`
    (ascending) before `.range`, so offset pages cannot overlap or omit
    rows. **FP-15-UNAVAILABLE**
    — A failed orders read MUST NOT be converted into `tableTotals: {}` or a
    seated `CHF 0.00`. On query error, `tableTotals` is `null`; `useFloorPlan`
    MUST NOT coerce `null` to `{}`; the overlay omits `billTotal` when totals
    are unavailable; the chip renders `CHF` only when `billTotal` is a number
    (a successful empty sum is `0` → `CHF 0.00`).
...
20. **WA-1 — Seven days, configured services only** — The overview displays
    ...
    copied onto a day that does not configure them. An empty or unconfigured
    `operating_windows` ledger (no rows, or a read error) MUST still render
    seven day columns with no services. The overview MUST NOT substitute
    `DEFAULT_OPERATING_DAYS` or any other suggested-segment template for
    missing configured days. Guest booking MAY keep a separate default-hours
    fallback; this overview must not.
...
26. **WA-7 — Isolate weekly load from current-day Dashboard** — Failure of
    weekly overview window or slot loading MUST NOT prevent the current-day
    FP-11 occupancy widgets or today's bookings from rendering. The overview
    MAY render seven empty-service columns when its load fails (including a
    `{ days: [] }` fallback). A throw or rejection from weekly
    `getAvailableSlots` / window reads MUST NOT reject the
    `getFloorSnapshot` / current-day path, whether those loads run
    concurrently or sequentially. Isolation is not satisfied by scanning the
    Dashboard source for a `Promise.all` shape or a `.catch` token; the
    failure path MUST be executed (weekly load rejects; current-day snapshot
    still settles for occupancy and today's bookings).
...
- **OH-SAVE-PATH — Immutable RPC namespace.**
  `public.replace_operating_windows(jsonb)` MUST remain `SECURITY INVOKER` and
  MUST set exact `search_path = ''`. Every runtime relation write MUST be
  schema-qualified: `DELETE FROM public.operating_windows` and
  `INSERT INTO public.operating_windows`. Every migration defining the
  RPC—currently `00000000000000_baseline.sql` and
  `20260818162000_operating_hour_segments.sql`—MUST carry those invariants. After
  a clean local reset, `pg_proc.proconfig` MUST contain `search_path=""` and
  `prosecdef` MUST remain false. `PUBLIC`, `anon`, and `authenticated` remain
  unable to execute it; `service_role` remains able to execute it; its atomic
  exact-payload persistence remains unchanged.
- **OH-SAVE-PATH-LINKED — Linked-project recorded-forward replay.** Linked
  project conformance is an operator-owned replay of the current idempotent
  `20260818162000` file after merge, even though its history row exists,
  followed by catalog, Advisor, and staff Save checks; do not repair the
  existing history row, `db push`, or reset linked history.
...
**19. SIB-PRIV — Server-mediated sibling table access** —
Staff access to sibling tables is mediated by guarded server actions using
`requireStaffUser` plus `createServiceClient`. RLS policies and SQL privileges
MUST agree.

`blocked_dates` MUST have no authenticated write policy. Table privileges MUST
`REVOKE ALL ON TABLE blocked_dates FROM PUBLIC, anon, authenticated` and then
`GRANT SELECT ON TABLE blocked_dates TO anon, authenticated` only. Public
SELECT RLS and `GRANT ALL ON TABLE blocked_dates TO service_role` stay.
Service-role `FOR ALL` RLS stays.

`tables`, `servers`, `table_merges`, `table_merge_members`, and
`status_events` MUST grant no table privilege to `PUBLIC`, `anon`, or
`authenticated` and MUST have no RLS policy for those roles. Each table keeps
RLS enabled, a service-role `FOR ALL` policy, and
`GRANT ALL ON TABLE <t> TO service_role`.
`event_inquiries` follows the same private-sibling recipe
([event-inquiries.md](./event-inquiries.md) EI-3); it is not a floor table.

Privilege recipes MUST use `REVOKE ALL ON TABLE <t> FROM PUBLIC, anon,
authenticated` followed only by the explicitly allowed public grant (`SELECT`
on `blocked_dates`; none on the private floor tables).

Canonical `00000000000000_baseline.sql` and `20260818180000_floor_tables.sql` /
`20260818193000_table_expected_minutes_and_merges.sql` MUST converge on this
surface after a complete local reset. Because the platform is pre-production
with no live users or data, do not add a dated compatibility migration;
linked-project conformance is an operator-owned reset and manual-UAT
(RES42-M1) per [docs/runbooks/deploy.md](../runbooks/deploy.md).

## Implementation trace (non-normative)

| Criterion                 | Shipped in                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Tests                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Guest note (§13)          | Column: `operating_windows.guest_note` — `supabase/migrations/00000000000000_baseline.sql`, `supabase/migrations/20260818162000_operating_hour_segments.sql`; `app/actions/availability.ts` (`WINDOW_COLUMNS`, `replace_operating_windows`). OH-NOTE-SAVE: `lib/reservations/operating-hours.ts` `MAX_GUEST_NOTE_LENGTH`; `validateOperatingDays` rejects a trimmed non-empty note over the cap (never truncate). OH-NOTE-INPUT: `components/staff/scheduling-manager.tsx` guest-note `<input maxLength={MAX_GUEST_NOTE_LENGTH}>`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | OH-NOTE-SAVE: `tests/unit/availability/actions.test.ts` → "accepts 240-character guest notes and rejects 241 before replace_operating_windows". OH-NOTE-INPUT: `tests/unit/scheduling/schema.test.ts` → "limits each scheduling guest-note input with the shared 240-character cap"                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| FP-9                      | `lib/floor/layout.ts` `floorDropCell` (`origin + delta / FLOOR_CELL_PX`, `FLOOR_CELL_PX` 120). `components/staff/floor-plan.tsx` `pointerDelta` is the only delta for the 8px threshold, merge highlight, and drop cell. In-drag position is `translate3d` on the drag-layer ref; React state updates only when the drop-target key changes. `touch-none` is only on the movable-chip class (`edit` + unlocked + not merging). Persist remains `updateTableState`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | `tests/unit/floor/layout.test.ts` → "floorDropCell resolves the nearest cell from the drag delta, preserving the grab offset"; `tests/unit/floor/drag-follow.test.ts` → "drop and merge highlight resolve from floorDropCell, not the absolute pointer cell"; "dragging chip translates by the pointer delta without per-move cell writes"; "movable chips opt out of browser panning; locked chips do not"; `tests/unit/floor/schema.test.ts` source-pins `floorDropCell`; `tests/e2e/floor/floor-drag.spec.ts` → "mouse drag follows the pointer, snaps to the nearest cell, and persists" (login helper `tests/e2e/helpers/staff-login.ts`). M-1 iPhone floor drag is manual-UAT (freshness 2026-09-24).                     |
...
| WA-1                      | `lib/floor/weekly-service-overview.ts` `buildWeeklyServiceOverview` — Monday-first `mondayContaining(selectedDate)` then seven `addCalendarDays`; `configuredServices` returns `[]` when the weekday is missing, `is_closed`, or has no segments. Loader `getConfiguredOperatingWindows()` (`app/actions/availability.ts`) — empty ledger or read error → `[]` (no `DEFAULT_OPERATING_DAYS`); `ledgerWeekdays` filter after `groupRowsByDay` drops DEFAULT-seeded weekdays. Guest `getAllOperatingWindowsMap` DEFAULT fallback unchanged. `/admin` default week is `getTodayInRestaurantTZ` unless `?week=` matches `DATE_RE`. Chrome: `components/staff/weekly-service-overview.tsx` (`data-testid="week-overview"`).                                                                                                                                                                                                                                                                                                                                                                                                                                                        | `tests/unit/floor/weekly-service-overview.test.ts` → "selected week lists seven days and only each day's configured services"; `tests/unit/floor/dashboard-weekly-overview.test.ts` → "admin Dashboard renders weekly service overview with week navigation"; "weekly overview does not load DEFAULT operating-day templates"                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| WA-7                      | `app/admin/page.tsx` `Promise.all` of `getAuthUser` / `getFloorSnapshot` / `loadWeeklyServiceOverview(selectedDate).catch(() => ({ days: [] }))` — isolate at the call site; do not switch the whole `all()` to `allSettled`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | `tests/unit/floor/dashboard-weekly-overview.test.ts` → "weekly overview rejection still settles tonight's floor snapshot"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| EARLY-PRIV (§17)          | `GRANT ALL ON TABLE blocked_dates TO service_role`, same for `reservations` and `menu_items` — `supabase/migrations/00000000000000_baseline.sql` (after each table's service_role RLS block, `-- REAZED-297`); `supabase/migrations/20260825140000_operating_windows_privilege.sql` (before `NOTIFY pgrst`; header names the siblings). Authenticated write policy and guest privileges: SIB-PRIV (§19) for `blocked_dates`; booking-rules AC-5 for `reservations`; menu-availability AC-2 for `menu_items`. Linked-remote apply stays runbook + manual-UAT.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | `tests/unit/scheduling/schema.test.ts` → "early-baseline tables grant ALL to service_role in baseline and privilege forward files"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| PUBLIC-READ-PRIV (§18)    | Guest SELECT on `blocked_dates` via SIB-PRIV (§19) `REVOKE ALL` then `GRANT SELECT` (same recipe for `menu_items`, menu-availability AC-2) — `supabase/migrations/00000000000000_baseline.sql`, `supabase/migrations/20260825140000_operating_windows_privilege.sql`, `supabase/migrations/20260827160000_public_catalog_privileges.sql` (`NOTIFY pgrst, 'reload schema'`). No authenticated write policy. Must not `GRANT SELECT ON TABLE reservations` (booking-rules AC-5). Linked-remote apply stays runbook + manual-UAT.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | `tests/integration/reservations/public-privileges.integ.test.ts` → "guest roles can SELECT blocked_dates only and no authenticated full-access policy remains"; "guest roles can SELECT menu_items only and no authenticated full-access policy remains"                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| SIB-PRIV (§19)            | `REVOKE ALL … FROM PUBLIC, anon, authenticated` then only the allowed public grant; drop/no-create authenticated policies; service-role `FOR ALL` + `GRANT ALL` — baseline plus `20260818180000_floor_tables.sql` / `20260818193000_table_expected_minutes_and_merges.sql` for private floor tables. `event_inquiries` uses the same recipe in baseline (event-inquiries EI-3). Staff reads/writes: `requireStaffUser` + `createServiceClient`. Linked conformance is operator-owned reset (RES42-M1), not a dated compatibility migration.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `tests/integration/security/sibling-privileges.integ.test.ts` → "local reset exposes only the approved sibling role capability matrix"; `tests/unit/scheduling/schema.test.ts` → "servers are seeded and service-role-only"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| FP-4 chip time            | `components/staff/floor-plan.tsx` dining-room chip — `{t.reservation.guestName}` and `{t.reservation.time}` inside `{t.reservation ? ( … ) : null}`; time on its own non-truncated `tabular-nums` line; `{t.label}` remains the heading; **FP-4-PARTY** party slot is gated `{t.reservation ? … {t.reservation.partySize} : null}` (no `?? t.seats`); chip size class still `tableChipSizeClass(t.seats)`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | `tests/unit/floor/schema.test.ts` → "floor chip renders the assigned reservation time without hiding the table label"; "floor chip party size does not fall back to table seats"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| FP-5                      | `app/actions/reservations.ts` `assignReservationTable` — `select("label, seats")`; `if (table && table.seats < party_size)` before overlap; unassign is a null table; `loadReservationOccupancyWindow` (settings `select` of `occupancy_duration_minutes, safety_buffer_minutes` plus the existing clamps) feeds that occupying window and staff `getReservationOccupancyWindow`. A `restaurant_settings` SELECT error is not an absent row: `assignReservationTable` logs and refuses a new non-null label with `errors.reservation.assignFailed`, writing nothing. Clearing the label stays allowed. Dropdown: `selectableTablesForAssignment(tables, partySize, currentLabel, occupancyBag)` in `TableAssignment` (`lib/reservations/selectable-tables.ts`); bag `Pick`s `AssignableReservation`; `claimedOccupyingLabels` skips `row.id === candidate.id` and uses bag minutes (omitted fields fall back to 90+15, no clamp in the helper). The dropdown may keep the 90+15 display fallback when the settings read fails. `app/admin/reservations/page.tsx` drills `occupancyWindow`; `TableAssignment` receives the unfiltered in-memory `reservations` list.           | `tests/unit/reservations/assign-table.test.ts` overlap + undersize; "refuses a new table label when restaurant_settings cannot be read"; `tests/unit/reservations/selectable-tables.test.ts` → "omits undersize tables except the reservation current label"; "includes a table claimed only by a non-overlapping occupying reservation"; "uses the configured occupancy duration and safety buffer for occupying windows"; `tests/unit/components/staff/table-assignment.test.ts` → "table assignment dropdown options follow the in-memory reservation list"                                                                                                                                                                  |
| CL-1                      | `validateOperatingDays(days, slotIntervalMinutes?)` rejects normalized duplicate times with `errors.scheduling.duplicateSlotTime` (per-segment `Set` of `normalizeTime(slot.time)`, after `invalidSlotMax`). HH:MM, half-open `[opens, closes)`, opens-aligned grid; empty `bookable_slots` skipped. A slotted `upsertOperatingWindows` validates on `restaurant_settings.slot_interval_minutes` (absent/null → 30; SELECT error → `errors.availability.settingsLoadFailed`, no RPC). `SchedulingManager` preview takes `slotIntervalMinutes` from the scheduling page. — `lib/reservations/operating-hours.ts`, `app/actions/availability.ts`, `components/staff/scheduling-manager.tsx`, `app/admin/scheduling/page.tsx`                                                                                                                                                                                                                                                                                                                                                                                                                                                    | `tests/unit/scheduling/cover-limits.test.ts` → "accepts on-grid service slots and rejects off-grid or outside-window times"; "rejects duplicate bookable-slot times in one segment"; `tests/unit/availability/upsert-slot-interval.test.ts` → "validates bookable slots on the configured slot-interval grid"; "fails closed when the slot interval cannot be read"; `tests/unit/components/staff/scheduling-manager-interval.test.ts` → "Save preview validates on the configured interval"                                                                                                                                                                                                                                    |
...
- `app/pos/page.tsx`
- `app/actions/operations.ts` (`getTables`, `getServers`)
