...
26. **WA-7 — Isolate weekly load from current-day Dashboard** — Failure of
    ...
    MAY render seven empty-service columns when its load fails. A throw from
    weekly `getAvailableSlots` / window reads MUST NOT reject the
    `getFloorSnapshot` / current-day path.
...
| WA-7                      | `app/admin/page.tsx` `Promise.all` of `getAuthUser` / `getFloorSnapshot` / `loadWeeklyServiceOverview(selectedDate).catch(() => ({ days: [] }))` — isolate at the call site; do not switch the whole `all()` to `allSettled`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | `tests/unit/floor/dashboard-weekly-overview.test.ts` → "weekly overview load is isolated from tonight's floor snapshot"                                                                                                                                                                                                       |
