...
| SIB-PRIV (§19)            | `REVOKE ALL … FROM PUBLIC, anon, authenticated` then only the allowed public grant; drop/no-create authenticated policies; service-role `FOR ALL` + `GRANT ALL` — baseline plus `20260818180000_floor_tables.sql` / `20260818193000_table_expected_minutes_and_merges.sql` for private floor tables. Staff reads/writes: `requireStaffUser` + `createServiceClient`. Linked conformance is operator-owned reset (RES42-M1), not a dated compatibility migration.                                                                                                                                                                 | `tests/integration/security/sibling-privileges.integ.test.ts` → "local reset exposes only the approved sibling role capability matrix"; `tests/unit/scheduling/schema.test.ts` → "servers are seeded and service-role-only"                                                         |
