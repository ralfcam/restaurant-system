...
9. **G-CR3 — US latest-head release gate** —
   ...
   `wrong_bot`. Login allow-list (`coderabbitai[bot]`) is unchanged. Issue and inline comments are identity
   surfaces only; they are not finding-routing sources. It MUST also fail
   closed on unresolved US CodeRabbit threads, rate-limit/billing event, or
...
handoff commands omit remote finding fields`, and adapter snapshot cases.
