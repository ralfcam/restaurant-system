...
23. **RES-TRIGGER-EXEC — Booking trigger is not a guest RPC.**
    ...
    converge on that ACL. The linked project, whose latest recorded function
    writer is currently `20260828121224_table_fit_availability.sql`, is updated
    by separately authorized execution of the latest idempotent
    function-defining forward even when its history row already exists; do not
    `db push` or reset its forked migration history.
