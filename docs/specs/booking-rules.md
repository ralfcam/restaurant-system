...
| AC-5 RES-PRIV | `REVOKE ALL` then `GRANT INSERT` on `reservations`; drop authenticated `FOR ALL`; `DROP POLICY IF EXISTS "Allow public read reservations"` (no `CREATE`); no `GRANT SELECT`; staff list/mutation including `getReservations` is `requireStaffUser` + `createServiceClient` — `supabase/migrations/00000000000000_baseline.sql`, `supabase/migrations/20260825140000_operating_windows_privilege.sql`, `supabase/migrations/20260827160000_public_catalog_privileges.sql`. Nullable `reservations.email` is extra PII on that insert-only table (post-visit-review-email PV-9). | `tests/integration/reservations/public-privileges.integ.test.ts` → "guest roles can INSERT reservations only and no authenticated full-access policy remains"; `tests/unit/reservations/get-range-service-client.test.ts`; `tests/integration/reservations/review-email-pii.integ.test.ts` → "anon cannot SELECT guest email on reservations" |
