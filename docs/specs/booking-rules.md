...
27. **BW-16 — Fully booked error stays in the confirmation form.** When
    ...
    hours, generic save failure) keep their existing surfaces.
...
| BW-16                    | `components/site/reservation-widget.tsx` `confirm()` exact-string branch `error === "Booking denied: This time is fully booked."` → `setFullyBookedError` (no `toast.error`, no `setStep`); step 2 `<p role="alert">{fullyBookedError}</p>`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `tests/unit/reservation-widget/fully-booked-error.test.ts` → "fully booked rejection is shown inside the confirmation form without a page toast"                                                                                                                                                                                                                                                                                                                                                                              |
