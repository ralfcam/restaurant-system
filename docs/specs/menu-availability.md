...
| MT-1 Staff tab writes       | `getMenuTabs` / `createMenuTab` / `renameMenuTab` / `reorderMenuTabs` are `requireStaffUser` + `createServiceClient` + `.from("menus")`; `getPublicMenuTabs` is `createAnonClient` — `app/actions/menu.ts`. No authenticated write/`FOR ALL` on `menus`.                                                                                                                                                                           | `tests/unit/menu/catalog-service-client.test.ts` → "staff menu tab mutations use createServiceClient after requireStaffUser"                                                                                                                                                                                                        |
| MT-2 Stable identity        | `renameMenuTab` `.update({ title, title_en }).eq("id", id)`; `reorderMenuTabs` `.update({ sort_order }).eq("id", tabId)` — `app/actions/menu.ts`. Existing `menu_items.menu_id` stays the tab `id`.                                                                                                                                                                                                                                | `tests/unit/menu/menu-tab-identity.test.ts` → "rename and reorder keep menu item menu_id"                                                                                                                                                                                                                                           |
...
| MT-4 menus PUBLIC-READ-PRIV | `CREATE TABLE menus` (`id`, `title`, `title_en`, `sort_order`) in `00000000000000_baseline.sql`; RLS; `"Allow public read menus"`; `DROP POLICY IF EXISTS "Allow authenticated full access to menus"` (no `CREATE`); `REVOKE ALL` then `GRANT SELECT` + `GRANT ALL TO service_role`. Same DROP + REVOKE/GRANT trio in `20260825140000_operating_windows_privilege.sql` and `20260827160000_public_catalog_privileges.sql`.         | `tests/integration/menu/menus-privileges.integ.test.ts` → "guest roles can SELECT menus only and no authenticated full-access policy remains"                                                                                                                                                                                       |
...
| MT-6 Reorder persists       | After `reorderMenuTabs`, a new read returns tabs in saved `sort_order` (PostgREST `.order("sort_order")` plus in-action JS sort).                                                                                                                                                                                                                                                                                                  | `tests/unit/menu/menu-tab-persistence.test.ts` → "reordered menu tabs are returned in saved sort_order"                                                                                                                                                                                                                             |
...
- `menus` table in `supabase/migrations/00000000000000_baseline.sql` (PUBLIC-READ-PRIV;
  same recipe as `menu_items`); seed ids `midi`, `soir`, `boissons`, `blanc`,
  `rouge` in `supabase/seed.sql`
