...
- **Schema:** `supabase/migrations/00000000000000_baseline.sql` — idempotent DDL for
  ...
  named policies before GRANT when `20260825140000` is already recorded), plus
...
| ----------- | ----------------------- | ------------- | -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Staff admin | `admin@test.local`      | `password123` | `11111111-1111-1111-1111-111111111111` | Local and linked non-prod (`db reset --local` and `--linked`). Signs in at `/auth/login`. JWT `raw_app_meta_data.role` is `"staff"` (not `raw_user_meta_data`, not `auth.users.role`). Staff routes `/admin`, `/pos`, `/kds` require `"staff"` or `"super_admin"`. Login email on both `auth.users.email` and `auth.identities` (`admin@test.local`). Empty `auth.users.email` is a real unique key in GoTrue (`users_email_partial_key`). Email pre-confirmed. Never seed production. |
| Super-admin | `superadmin@test.local` | `password123` | `22222222-2222-2222-2222-222222222222` | Same reset scope as staff. Distinct id. JWT `raw_app_meta_data.role` is `"super_admin"` (not GoTrue `is_super_admin`). Login email on both `auth.users.email` and `auth.identities` (`superadmin@test.local`), pairwise distinct from staff. `ON CONFLICT DO NOTHING`. Never seed production.                                                                                                                                                                                          |
