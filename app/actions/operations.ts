...
function mapTable(
  ...
  return {
    ...
    expectedMinutes: clampExpectedMinutes(
      Number(row.expected_minutes ?? DEFAULT_EXPECTED_MINUTES),
    ),
    x: Number(row.x),
    y: Number(row.y),
...
async function loadMergeContext(
  ...
  if (membershipError) {
    ...
    const fallback = (await loadMergeEvents(db)).find((merge) =>
      merge.tableIds.includes(tableId),
    )
...
async function dissolveMerge(db: ServiceDb, mergeId: string) {
  const current = (await loadMergeEvents(db)).find(
...
export async function getTables(): Promise<PersistedTable[]> {
  ...
  if (error) {
    console.error("[operations] getTables:", error.message)
    return []
  }
  const { data: members, error: memberError } = await db
    .from("table_merge_members")
    .select("merge_id, table_id")
  let mergeByTable = new Map(
    (members ?? []).map((row) => [String(row.table_id), String(row.merge_id)]),
  )
  if (memberError) {
    console.error("[operations] getTables members:", memberError.message)
    mergeByTable = new Map(
      (await loadMergeEvents(db)).flatMap((merge) =>
        merge.tableIds.map((id) => [id, merge.id] as const),
      ),
    )
  }
  const mapped = (data ?? []).map((row) =>
    mapTable(row, mergeByTable.get(String(row.id)) ?? null),
  )
...
export async function getActiveMerges(): Promise<PersistedMerge[]> {
  ...
  return (merges ?? []).flatMap((merge) => {
    const mergeMembers = (members ?? [])
      ...
      .filter((row): row is { id: string; label: string; seats: number } =>
        Boolean(row),
      )
      .map((row) => ({
        id: String(row.id),
...
export async function updateTableState(input: {
  ...
  const merge =
    input.status !== undefined || input.expectedMinutes !== undefined
    ...
    if (merge) {
      if (dissolvesMerge(input.status)) {
        ...
        if (restartsMergeClock(input.status)) {
          patch.expires_at = mergeExpiresAt(
            now,
            merge.expectedMinutes,
          ).toISOString()
        }
        const { error } = await db
          .from("table_merges")
...
export async function mergeTables(input: {
  ...
  try {
    ...
    const mapped = rows.map((row) => {
      const membership = (existing ?? []).find(
        (member) => String(member.table_id) === String(row.id),
      )
    ...
    if (mergeIds.length === 1) {
      ...
      const { error: memberError } = await db
        .from("table_merge_members")
        .insert(
          newcomers.map((table) => ({ merge_id: mergeId, table_id: table.id })),
        )
      if (memberError) {
      ...
      const expectedMinutes = clampExpectedMinutes(
        input.expectedMinutes ??
          defaultMergeExpectedMinutes(allMembers.length ? allMembers : mapped),
      )
      const now = new Date()
      const expiresAt = mergeExpiresAt(now, expectedMinutes).toISOString()
      ...
      revalidatePath("/admin/floor")
      return mapMerge(
        { ...merge, expected_minutes: expectedMinutes, expires_at: expiresAt },
        allMembers,
      )
    }
...
async function mergeUsingEvents(
  ...
  const touching = active.filter((merge) =>
  ...
  if (touching.length === 1) {
    ...
    const newcomers = mapped.filter(
      (table) => !merge.tableIds.includes(table.id),
    )
    const reason = canAddTablesToMerge({ status: merge.status }, newcomers)
    if (reason) return { error: reason }
    const tableIds = [
      ...new Set([...merge.tableIds, ...newcomers.map((table) => table.id)]),
    ]
    const catalog = [
      ...mapped,
      ...merge.tableIds
    ...
    const expectedMinutes = clampExpectedMinutes(
      expectedMinutesInput ??
        defaultMergeExpectedMinutes(members.length ? members : catalog),
    )
...
export async function createKitchenOrder(input: {
  table: string
  server: string
...
export async function getActiveKitchenOrders(): Promise<KdsOrder[]> {
  ...
  const { data, error } = await db
    ...
    .order("created_at", { ascending: true })
  if (error) {
    console.error("[operations] getActiveKitchenOrders:", error.message)
    return []
  }
  return (data ?? []).map((row) => ({
    id: row.id,
    ...
    placedAtMs: new Date(row.created_at).getTime(),
    status: row.status as KdsOrder["status"],
    lines: (row.order_items ?? []).map(
      (item: {
        menu_item_id: string
        item_name: string
        quantity: number
...
const ORDER_TRANSITIONS: Record<
  KdsOrder["status"] | "completed" | "cancelled" | "voided",
  string[]
> = {
  new: ["preparing", "cancelled", "voided"],
