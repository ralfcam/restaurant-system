...
import { useChefsPicks, type ChefsPicksPayload } from "@/hooks/use-chefs-picks"

export function HomePageClient({
  initialChefsPicks,
}: {
  initialChefsPicks: ChefsPicksPayload
}) {
  const t = useTranslations()
  const locale = useLocale()
  const { heroImageUrl } = useRestaurantHeroImage()
  const restaurantInfo = useRestaurantInfoBar()
  const { enabled: chefsPicksEnabled, items: featured } =
    useChefsPicks(initialChefsPicks)
