...
export function isReservationDueForAssignment(
  reservation: Pick<AssignableReservation, "date" | "time" | "status" | "table_label">,
...
function compareDueReservations(a: AssignableReservation, b: AssignableReservation): number {
...
export function planAutoAssignments(
  ...
  const due = reservations
    .filter((reservation) => isReservationDueForAssignment(reservation, now, leadMinutes))
