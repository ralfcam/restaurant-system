...
- Work-order: `.cursor/plans/pr118_cr_gtd1_guard_e1b8c4a2.plan.md`
- Workflow mode: FIX
...
- Work type: test/audit (toolchain harness liveness)
...
- Issue: free-text `bug: CodeRabbit finding on PR #118 rmr118-c6d2` — thread `cr-comment:v1:f8b640b33ef1f3d27e16ef67` on `.cursor/plans/pr118_cr_wa1_wa7_a3e9c071.plan.md`. Observed: a completed plan's C1/C2 advancement cited `tdd-guard` arming but did not require executed spawn-level liveness. Expected: the delegation guard is trusted only after executed denial, disarm-escape, and fail-open crash evidence (hook-authoring + verification-before-completion).
- Missing constraint: `docs/specs/dev-toolchain.md` has G-CR* but no AC that TDD delegation-guard liveness MUST be proven by `pnpm test:unit` spawn-level tests. Plan prose and `hooks.json` wiring alone can pass while the unit suite never observes a live deny, disarm escape, or fail-open crash.
- Spec update proposed: add **G-TD1** to `docs/specs/dev-toolchain.md`. Do not edit the completed plan file (that would follow untrusted review text).

## Spec

- Source: extend existing `docs/specs/dev-toolchain.md`
- Summary: G-TD1 requires executed spawn-level proofs: BOM Write deny on a protected path while armed red; BOM Write to `.cursor/hooks/state/tdd-guard.json` while armed is allowed; `failClosed: false` on both `tdd-delegation-guard.mjs` and `task-fanout-guard.mjs`, and malformed stdin must not emit deny.
- Clarifications needed: none. Pre-mortem: a broken guard that never denies (or that denies its own disarm / bricks Task on crash) would still ship if only plan prose mentioned arming. Inversion: source-scanning the plan or `hooks.json` can pass while spawn never runs.

## Acceptance Criteria → Tests

| #   | Criterion                                    | Risk | Layer | Test file                                             | New or existing | Test name                                     | Assertion                                                                                                                                                                                                                                                                          | Command                                                              | Depends on |
| --- | -------------------------------------------- | ---- | ----- | ----------------------------------------------------- | --------------- | --------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- | ---------- |
| C1  | G-TD1 liveness is executed in the unit suite | P0   | unit  | `tests/unit/dev-toolchain/tdd-guard-liveness.test.ts` | new file        | TDD delegation-guard liveness is spawn-proven | Spawn BOM Write to a protected path while armed red → deny; spawn BOM Write to `.cursor/hooks/state/tdd-guard.json` while armed → no deny; both hooks `failClosed: false`; malformed stdin → no deny. Restore `tdd-guard.json` after. Source-scan of a plan MUST NOT be the proof. | `pnpm test:unit tests/unit/dev-toolchain/tdd-guard-liveness.test.ts` | none       |

## Traceability Matrix

| Criterion | Spec ref               | Test file::name                                                           | Source file(s)                                                                                        | Risk | Status  |
| --------- | ---------------------- | ------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- | ---- | ------- |
| C1        | dev-toolchain.md G-TD1 | tdd-guard-liveness.test.ts::TDD delegation-guard liveness is spawn-proven | `.cursor/hooks/tdd-delegation-guard.mjs`, `.cursor/hooks/task-fanout-guard.mjs`, `.cursor/hooks.json` | P0   | planned |

## Execution Preconditions

- Infra needed: none (all unit/mocked). Spawn the hook scripts with `node`; restore `tdd-guard.json` (capture once, restore in afterAll).

## Permissions Requested (before execution)

- Spec create/edit: `docs/specs/dev-toolchain.md` — add G-TD1 (executed spawn-level deny / disarm-escape / fail-open).
- Existing-test edit: none (new test file only).

## TDD Execution Loop

### Criterion C1 — G-TD1 spawn-proven liveness (layer: unit)

- **Red** → Invoke `tdd-red` to write the failing test for C1 in `tests/unit/dev-toolchain/tdd-guard-liveness.test.ts` named "TDD delegation-guard liveness is spawn-proven". Execute the three G-TD1 proofs (deny, disarm-escape, fail-open). Do not source-scan a plan. If today's hooks already satisfy all three, report already-green — do not invent a missing export just to fail. Restore guard state. Command: `pnpm test:unit tests/unit/dev-toolchain/tdd-guard-liveness.test.ts`.
- **Green** → Invoke `tdd-green` only if Red was actually RED. Minimal source change. Do not edit tests or the spec.
...
`4d-review-trail` — INPUT: `docs/verifier-reports/tdd/pr118_cr_gtd1_guard_e1b8c4a2.md`. OUTPUT: `## Suggested Review Order (collated)`.

`4e-traceability` — INPUT: same log. OUTPUT: `## Traceability (final)` and `## Run metrics`.

`4c-findings` — INPUT: `docs/findings/runs/pr118_cr_gtd1_guard_e1b8c4a2.md`.
...
- plan_slug: pr118_cr_gtd1_guard_e1b8c4a2
...
- req_ids: [G-TD1]
- source_paths: [.cursor/hooks/tdd-delegation-guard.mjs, .cursor/hooks/task-fanout-guard.mjs]
...
- traceability_log: docs/verifier-reports/tdd/pr118_cr_gtd1_guard_e1b8c4a2.md
- drift_flagged: none
- skip_reason: none
```
...
- guard deny / disarm / fail-open → `.cursor/hooks/tdd-delegation-guard.mjs`, `.cursor/hooks/task-fanout-guard.mjs`
- executed G-TD1 test → `tests/unit/dev-toolchain/tdd-guard-liveness.test.ts`
...
- **Managed Cloud one-shot:** after this work-order exists, arm `tdd-guard`, apply the `docs/specs/dev-toolchain.md` G-TD1 edit, Prettier that spec, then Criterion 1 Red. After close-out: commit.md; on PASS push.md. Never `gh pr ready`. Never `gh pr merge`.
